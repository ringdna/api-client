...
export const RawCompletionPromise: unique symbol = Symbol(
...
type Authenticator = (
  client: ApiClient
) => Promise<void | { headers: JsonObject }> | void | { headers: JsonObject };
export type OnRequestFail = (
...
export type MethodOptionsRealized = Omit<
  MethodOptions<ParamsGeneric>,
  "path"
> & {
  path: string;
  independent: boolean;
...
export type Executor = (
  resource: ResourceGeneric,
  client: ApiClient
) => Promise<ResourceGeneric>;
export type ApiClient = {
  execute: Executor;
  cache: ClientCache;
};

export type Loading<Payload, Params> =
