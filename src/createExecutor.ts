...
function createExecutor(apis: ApiConfigs): Executor {
  return async (resource: ResourceGeneric, client: ApiClient) => {
    if (resource.stage !== RequestStage.Latent)
      throw new Error(
        "Api Client cannot execute a resouce which is not in latent stage"
      );
    ...
    let p = new Promise<ResourceGeneric>(async (resolve, reject) => {
      ...
      let auth =
        apiConfig.authenticator && (await apiConfig.authenticator(client));
      ...
      let url = `${apiConfig.basePath}${path}${
        paramsQuery ? `?${urlSearchParams.toString()}` : ""
      }`;
      if (apiConfig.debug)
        console.log(`## client fetch ${url}`, { headers, method, body });
