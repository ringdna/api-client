...
function createUseFetch<Payload, Params extends ParamsGeneric = void | null>(
  methodOptions: MethodOptions<Params>
) {
  return (
    params: Params,
    hookOptions: HookOptions = {}
  ): ReturnTuple<Payload, Params> => {
    methodOptions.method = methodOptions.method || HttpMethods.Get;
    let independent =
      methodOptions.independent === undefined
        ? methodOptions.method !== HttpMethods.Get
        : methodOptions.independent;
    ...
    let cacheKey = useMemo(() => {
      return (
        ...
        inferCacheKey(methodOptions.key, cacheType, independent, paramsString)
      );
    ...
    if (!cache.isReady && !initRef.current)
      initRef.current = cache.readyPromise.then(forceRender);
    ...
    useEffect(() => {
      ...
      return cache.subscribe(() => {
        ...
        if (
          cachedResource !== resourceRef.current ||
          cache.renewTime !== _renewTime
        ) {
    ...
    let refetch: Refetch<Payload, Params> = useCallback(
      (overrideParams?: Params, overrideCacheKey?: string) => {
        ...
        let realizedPath =
          typeof methodOptions.path === "function"
            ? methodOptions.path(overrideParams || params)
            : methodOptions.path;
    ...
    let needsRefetchFromInterval =
      resource &&
      refetchInterval &&
      resource.timestamp < Date.now() - refetchInterval;
    let needsRefetchFromFlag =
      resource && resource.flag === ResourceFlag.NeedsRefetch;
    // @TODO add option to not do init
    let needsInitFetch = resource && resource.timestamp < cache.renewTime;
    let fetchSync =
      !paused &&
      (!resource ||
        needsRefetchFromFlag ||
        needsRefetchFromInterval ||
        needsInitFetch);
    ...
    let completionPromise = resourceRef.current?.[RawCompletionPromise] as
    ...
    return [
      ...
      failTimestamp && (!successTimestamp || failTimestamp > successTimestamp)
        ? fail
        : undefined,
...
function createUseFetchAction<
  Payload,
  Params extends ParamsGeneric = void | null
>(methodOptions: MethodOptions<Params>) {
